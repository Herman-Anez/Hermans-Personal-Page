...
export async function generateStaticParams({ params }: { params: { locale: string } }): Promise<{ slug: string }[]> {
  const posts = getPosts(["src", "app", "[locale]", "work", "projects"], params?.locale || "es");
...
export async function generateMetadata({
  ...
  const { work } = getContent(dict, locale);
...
export default async function Project({
  ...
  const dict = getDictionary(locale);
  const { work, person, about } = getContent(dict, locale);

  let post = getPosts(["src", "app", "[locale]", "work", "projects"], locale).find((post) => post.slug === slugPath);
